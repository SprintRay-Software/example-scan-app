...
**本地扫描历史。** 本示例会保留已经发送过的扫描件,**只**以 `externalCaseId` 为键 ——
这样再次拉起一个你已经发送过的 case 时,打开的是存下来的那次扫描,而不是重新扫一遍。
该键只在 `externalCaseId` 有值、且与 `case.ID` 不同时才生效 —— 与会话标识相等的 id 并不能标识出任何 case;
同时它必须是一个朴素 token:最长 100 个字符,首字符为字母或数字,其余只允许字母、数字、`.`、`_`、`-`。
不满足的一律直接拒绝,而不是清洗后使用 —— 这个 id 会成为应用状态目录下的一个目录名,
而 payload 里的字段绝不允许左右本应用往哪里写。SprintRay Web 端不下发 `externalCaseId`,
因此真实的 SprintRay 拉起不会触发历史,流程与此前完全一致。

只有在**所有**请求的上传都成功、**且** scan-finish 调用也返回之后,才会写入一条记录。会话未关闭之前
SprintRay 侧没有已完成的扫描任务,此时若因 finish 失败仍写入记录,就会对一个根本打不开的 case 宣称
"已发送" —— 未命中的代价只是重扫一次,而错误命中的代价是此后每一次命中都不再可信。牙龈/牙位网格的上传
刻意不计入这道门槛:它们属于会话元数据,不影响扫描件本身的完整性,而且最多 34 个链接,是整个流程中最容易
出问题的一步。

命中之后的行为按端区分,这是有意为之。桌面 UI 打开存下来的扫描后就停在那里:新的一次拉起带的是**新的**
一次性 code,把它花掉就等于开启了一个没人要求的真实会话,所以是否重发仍然由医生决定。
CLI 则会报出命中,然后照常执行流程 —— 敲下这条命令本身就是请求,而重新发送一个已知 case
正是文档里给集成方的路径。历史数据只存在于本机安装内:不会上传,也不改变任何上行内容。
...
- `externalScanFileType`:**每次上传必传。** 即**你自己对这个文件的命名** —— `UpperArch`、`LowerJaw`、
  ...
  **你的应用会用到的名字清单**交给 SprintRay
  ...
  但 SprintRay 存下来的是它第一次见到的写法,因此每次都用同一种拼写。(该字段不会以 `400` 强制校验 ——
  不带它的上传照样成功,好让这套契约之前写好的客户端继续可用。它是**对你这个集成的要求**:不传它,
  文件类型就无从映射。)
